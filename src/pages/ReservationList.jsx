  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        const meetingResponse = await axios.post('/api/google-calendar/create-event-with-meeting', {
        ...
        setMeetingLink(meetingResponse.data.meetingLink || '링크 생성 실패');
      } catch (error) {
  ...
  return (
    <R.Container>
      ...
      <R.Consulting>
        ...
        <R.CLink href={meetingLink} target="_blank" rel="noopener noreferrer">{meetingLink || '링크 생성 중...'}</R.CLink>
      </R.Consulting>
      <R.ConsultingContent>
        <R.D>
          <R.De>디자이너</R.De>
      ...
      {showPopup && (
        <R.PopupOverlay onClick={handleClosePopup}>
          <R.Popup>
            ...
            <R.PopupButtonGroup>
              <R.PopupButton primary onClick={handleRCancelClick}>예약 취소</R.PopupButton>
